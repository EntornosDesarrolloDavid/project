<?php
	class CAplicacion
	{
		...
		public function __construct($config)
		{
			...
			//Se crea la instancia de CSesion y si en el fichero config la variable controlAutomatico es true
			//se inicializa
			$this->_sesion=new CSesion();

			if (isset($config["sesion"])) {
				if ($config["sesion"]["controlAutomatico"]) {
			...
			//Se crea la instancia de CAcceso
			...
			//Si se han establecido las variables de la base de datos se inicializa la ACLBD con esos parámetros
			if (isset($config["ACL"])) {
				if ($config["ACL"]["controlAutomatico"]) {
					$this->_aclbd = new CACLBD($config["BD"]["servidor"],
					$config["BD"]["usuario"],
					$config["BD"]["contra"],
					$config["BD"]["basedatos"]);
				}
		...
		public function paginaError($numError,$mensaje=null)
		{
			...
			if (file_exists($ruta))
			    ...
			    {
			    	echo "Error $numError: $mensaje<br>";
